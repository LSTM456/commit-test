...
from data_processing.data_process import load_data
setup_seed(1234)
...
optimizer_grouped_parameters = [
    {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay)],
        'weight_decay_rate': 0.01}, # 第一组参数是整个模型的所有参数，并给出了统一的权重衰减系数
    {'params': [p for n, p in param_optimizer if any(nd in n for nd in no_decay)],
        'weight_decay_rate': 0.0}] # 二组参数是指定了名称的特殊参数，并将其权重衰减系数设为 0。这样就可以保证特殊的参数不会被加入权重衰减中。
